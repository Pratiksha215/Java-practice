		...
		while (!exit) {
			...
			case 3:
				...
				if (counter < basket.length) {
					...
					System.out.println("Basket full!!!!!!");
				break;
			case 4: // Display taste of all fruits in the basket n invoke sub class specific func. :
					// for-each
				// basket : {m1,m2,a1,o1,o2.....null}
				System.out.println("Displaying Fruit Basket");
				for (Fruit f : basket) // f=basket[0],f=basket[1].....
					if (f != null) {
						f.taste();// javac : chks it by type of the ref : Fruit: JVM :
						/*
						 * if (f instanceof Apple) ((Apple) f).jam(); else if (f instanceof Orange)
						 * ((Orange) f).juice(); else ((Mango) f).pulp();
						 */
						if (f instanceof Mango)//correct usage of instanceof
							((Mango) f).pulp();// class cast exc
						else
							System.out.println("Invalid fruit : can't be pulped!!!!!!");
					}
